            ...
            <div class="flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <div>
                    <button
                        ...
                        aria-expanded="false"
                        on:click=move|_|{set_menu_open.update(|d|{ *d = !*d;});}
                    >
                    ...
                    <div class="fixed right-1 z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-sm dark:bg-gray-800 dark:divide-gray-600" id="navbar-hamburger" class:hidden=move||!*menu_open.read()>
                        <ul class="flex flex-col font-medium mt-4 rounded-lg bg-gray-50 dark:bg-gray-800">
                            <li>
                                <label class="relative flex items-center group p-2 text-xl">
                                    Dark Mode
                                    <input type="checkbox" class="absolute left-1/2 -translate-x-1/2 w-full h-full peer appearance-none rounded-md" prop:checked=move||dark_mode.0.read().0 on:change=move|ev|{
